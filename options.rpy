...
define config.name = _("Library of sins")
...
define gui.about = _p("""
ОБ ИГРЕ: БИБЛИОТЕКА ГРЕХОВ
ГЛАВА ПЕРВАЯ - ГНЕВ

ОТ АВТОРА
Спасибо Вам за то, что решили погрузиться в мир Библиотеки Грехов. Как инди-разработчик, я искренне ценю Ваше время. Этот проект является результатом самостоятельного изучения геймдизайна, нарративного дизайна и основ программирования.

Своей главной задачей я видел создание глубокой атмосферы психологического хоррора, где каждый выбор игрока (и каждый грех) имеет значение.

ПОРТФОЛИО
Эта глава создана с целью демонстрации навыков в разработке интерактивного повествования и структурировании сценария с ветвлениями.

Вся игровая логика и сценарий написаны с использованием движка Ren’Py.
[Добавьте, если хотите подчеркнуть техническую сторону]: Взаимодействие с ИИ и Стелс-механики были дополнительно прототипированы в Unreal Engine (Blueprints/C++).

БЛАГОДАРНОСТЬ
* Семье и друзьям за поддержку в освоении сложного пути разработчика.
* Сообществу открытого кода и создателям Ren'Py за предоставленные инструменты.

Автор идеи, сценарий, код и дизайн: [ВАШЕ ИМЯ ИЛИ НИКНЕЙМ]
Дата завершения: [ТЕКУЩИЙ МЕСЯЦ И ГОД]

фоновая музыка и звуковые эффекты в Главе 1 - 'Гнев' предоставлены:

Композитор: Кевин МакЛеод (Kevin MacLeod)

"Сайт: incompetech.com

Лицензия: Creative Commons Attribution (CC BY 4.0)

Треки: Oppressive Gloom


Использованные картирки:
...
define build.name = "Library of sins"
...
define config.save_directory = "MyyyGreatNovel-1763387060"
...
define config.window_icon = "gui/window_iconn.jpg"
